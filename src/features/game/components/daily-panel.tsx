    ...
    <section className="space-y-6">
      <Card className="overflow-hidden border-white/8 bg-zinc-900/70 py-0 shadow-none">
        ...
        <CardContent className="grid gap-px bg-white/6 p-0 sm:grid-cols-3">
          <DailyMetric
            label="Thức ăn"
            value={`${foodCount} khẩu phần`}
            detail={foodCount > 4 ? "Vẫn còn đủ cho cả nhóm" : "Nguồn dự trữ đang xuống thấp"}
            tone={foodCount <= 4 ? "warning" : "neutral"}
          />
          <DailyMetric
            label="Nước sạch"
            value={`${waterCount} chai`}
            detail={waterCount > 4 ? "Nguồn nước đang ổn định" : "Chỉ đủ dùng trong thời gian ngắn"}
            tone={waterCount <= 4 ? "warning" : "neutral"}
          />
          <DailyMetric
            label="Tinh thần nhóm"
            value={groupSanity >= 60 ? "Ổn định" : groupSanity >= 40 ? "Tạm ổn" : "Bất ổn"}
            detail={`Trung bình ${groupSanity}/100`}
            tone={groupSanity < 60 ? "warning" : "neutral"}
          />
        </CardContent>
      </Card>
